...
public class MyServceImpl implements MyService {
	...
	public boolean checkValidConsent(GetRequest getRequest) {
		...
		if (clientConsentMappingEntity != null) {
			logger.debug("Consent record found: State={}, ConsentStatus={}", clientConsentMappingEntity.getState(),
					clientConsentMappingEntity.getConsentStatus());
	...
	private ClientConsentMappingDTO mergeConsentRequestAndResponse(ConsentNewRunRequest request,
		...
		logger.debug(
				"Merging ConsentNewRunRequest and ConsentNewRunResponse | clientUserId: {} | pan: {} | requestId: {} | consentHandle: {}",
				request.getUser().getClientUserId(), request.getUser().getPan(), response.getRequestId(),
	...
	private ClientConsentMappingDTO mergeRecurringRequestAndResponse(
		...
		logger.debug(
				"Merging RecurringNewRunRequest and Response | clientCode: {} | pan: {} | existingConsentHandle: {} | newRequestId: {}",
