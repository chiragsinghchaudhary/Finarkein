...
public class MyServceImpl implements MyService {
	...
	// ===================== Deposit Profile =====================
	@Override
	public List<DepositHolderDTO> getAllDepositProfiles() {
		return depositProfileRepo.findAll().stream().map(this::toDepositProfileDTO).collect(Collectors.toList());
	}

	@Override
	public Optional<DepositHolderDTO> getDepositProfileById(Long id) {
		return depositProfileRepo.findById(id).map(this::toDepositProfileDTO);
	}

	@Override
	public DepositHolderDTO saveDepositProfile(DepositHolderDTO dto) {
		DepositHolderEntity entity = toDepositProfileEntity(dto);
		return toDepositProfileDTO(depositProfileRepo.save(entity));
	}

	@Override
	public DepositHolderDTO updateDepositProfile(Long id, DepositHolderDTO dto) {
		DepositHolderEntity entity = depositProfileRepo.findById(id).orElseThrow();
	...
	public List<DepositSummaryDTO> getAllDepositSummaries() {
		return depositSummaryRepo.findAll().stream().map(this::toDepositSummaryDTO).collect(Collectors.toList());
	}
	...
	public List<DepositTransactionDTO> getAllDepositTransactions() {
		return depositTransactionRepo.findAll().stream().map(this::toDepositTransactionDTO)
	...
	// ===================== Equity Profile =====================
	@Override
	public List<EquityHolderDTO> getAllEquityProfiles() {
		return equityProfileRepo.findAll().stream().map(this::toEquityProfileDTO).collect(Collectors.toList());
	}

	@Override
	public Optional<EquityHolderDTO> getEquityProfileById(Long id) {
		return equityProfileRepo.findById(id).map(this::toEquityProfileDTO);
	}

	@Override
	public EquityHolderDTO saveEquityProfile(EquityHolderDTO dto) {
		EquityHolderEntity entity = toEquityProfileEntity(dto);
		return toEquityProfileDTO(equityProfileRepo.save(entity));
	}

	@Override
	public EquityHolderDTO updateEquityProfile(Long id, EquityHolderDTO dto) {
		EquityHolderEntity entity = equityProfileRepo.findById(id).orElseThrow();
	...
	public List<EquitySummaryDTO> getAllEquitySummaries() {
		return equitySummaryRepo.findAll().stream().map(this::toEquitySummaryDTO).collect(Collectors.toList());
	}
	...
	public List<EquityTransactionDTO> getAllEquityTransactions() {
		return equityTransactionRepo.findAll().stream().map(this::toEquityTransactionDTO).collect(Collectors.toList());
	}
	...
	// ===================== MF Profile =====================
	@Override
	public List<MFHolderDTO> getAllMFProfiles() {
		return mfProfileRepo.findAll().stream().map(this::toMFProfileDTO).collect(Collectors.toList());
	}

	@Override
	public Optional<MFHolderDTO> getMFProfileById(Long id) {
		return mfProfileRepo.findById(id).map(this::toMFProfileDTO);
	}

	@Override
	public MFHolderDTO saveMFProfile(MFHolderDTO dto) {
		MFHolderEntity entity = toMFProfileEntity(dto);
		return toMFProfileDTO(mfProfileRepo.save(entity));
	}

	@Override
	public MFHolderDTO updateMFProfile(Long id, MFHolderDTO dto) {
	...
	public List<MFSummaryDTO> getAllMFSummaries() {
		return mfSummaryRepo.findAll().stream().map(this::toMFSummaryDTO).collect(Collectors.toList());
	}
	...
	public List<MFTransactionDTO> getAllMFTransactions() {
		return mfTransactionRepo.findAll().stream().map(this::toMFTransactionDTO).collect(Collectors.toList());
	}
	...
	public List<ClientConsentMappingDTO> getAllClientConsents() {
		return clientConsentRepo.findAll().stream().map(this::toClientConsentDTO).collect(Collectors.toList());
	}
	...
	public List<ClientConsentMappingHistDTO> getAllClientConsentHistories() {
		return clientConsentHistRepo.findAll().stream().map(this::toClientConsentHistDTO).collect(Collectors.toList());
	}
	...
	// ===================== Conversion Methods =====================
	// Deposit Profile
	private DepositHolderDTO toDepositProfileDTO(DepositHolderEntity e) {
		return new DepositHolderDTO(e.getId(), e.getAccountNumber(), e.getAccountType(), e.getBankName(),
				e.getBranch(), e.getIfsc(), e.getNominee(), e.getPan(), e.getEmail(), e.getMobile(), e.getKycStatus());
	}

	private DepositHolderEntity toDepositProfileEntity(DepositHolderDTO d) {
		DepositHolderEntity e = new DepositHolderEntity();
		e.setId(d.id());
	...
	private DepositSummaryDTO toDepositSummaryDTO(DepositSummaryEntity e) {
		return new DepositSummaryDTO(e.getId(), e.getAccountNumber(), e.getCurrentBalance(), e.getInterestRate(),
				e.getMaturityDate());
	}
	...
	private DepositTransactionDTO toDepositTransactionDTO(DepositTransactionEntity e) {
		return new DepositTransactionDTO(e.getId(), e.getStartDate(), e.getEndDate(), e.getAmount(),
				e.getCurrentBalance(), e.getMode(), e.getNarration(), e.getReference(), e.getTransactionId(),
				e.getTransactionTimestamp(), e.getType(), e.getValueDate());
	}
	...
	// Equity Profile
	private EquityHolderDTO toEquityProfileDTO(EquityHolderEntity e) {
		return new EquityHolderDTO(e.getId(), e.getAddress(), e.getDematId(), e.getDob(), e.getEmail(),
				e.getKycCompliance(), e.getLandline(), e.getMobile(), e.getName(), e.getNominee(), e.getPan());
	}

	private EquityHolderEntity toEquityProfileEntity(EquityHolderDTO d) {
		EquityHolderEntity e = new EquityHolderEntity();
		e.setId(d.id());
	...
	private EquitySummaryDTO toEquitySummaryDTO(EquitySummaryEntity e) {
		return new EquitySummaryDTO(e.getId(), e.getCurrentValue(), e.getHoldingMode(), e.getIsin(),
				e.getIsinDescription(), e.getIssuerName(), e.getLastTradedPrice(), e.getUnits());
	}
	...
	private EquityTransactionDTO toEquityTransactionDTO(EquityTransactionEntity e) {
		return new EquityTransactionDTO(e.getId(), e.getStartDate(), e.getEndDate(), e.getCompanyName(),
				e.getEquityCategory(), e.getExchange(), e.getIsin(), e.getIsinDescription(), e.getNarration(),
				e.getOrderId(), e.getRate(), e.getTransactionDateTime(), e.getTxnId(), e.getType(), e.getUnits());
	}
	...
	// MF Profile
	private MFHolderDTO toMFProfileDTO(MFHolderEntity e) {
		return new MFHolderDTO(e.getId(), e.getAddress(), e.getDematId(), e.getDob(), e.getEmail(), e.getFolioNo(),
				e.getKycCompliance(), e.getLandline(), e.getMobile(), e.getName(), e.getNominee(), e.getPan());
	}

	private MFHolderEntity toMFProfileEntity(MFHolderDTO d) {
		MFHolderEntity e = new MFHolderEntity();
		e.setId(d.id());
		e.setAddress(d.address());
		e.setDematId(d.dematId());
		e.setDob(d.dob());
	...
	private MFSummaryDTO toMFSummaryDTO(MFSummaryEntity e) {
		return new MFSummaryDTO(e.getId(), e.getCostValue(), e.getCurrentValue(), e.getFatcaStatus(), e.getAmc(),
				e.getAmfiCode(), e.getClosingUnits(), e.getFolioNo(), e.getIsin(), e.getIsinDescription(),
				e.getLienUnits(), e.getLockinUnits(), e.getNav(), e.getNavDate(), e.getRegistrar(),
				e.getSchemeCategory(), e.getSchemeCode(), e.getSchemeOption(), e.getSchemeTypes(), e.getUcc());
	}
	...
	private MFTransactionDTO toMFTransactionDTO(MFTransactionEntity e) {
		return new MFTransactionDTO(e.getId(), e.getStartDate(), e.getEndDate(), e.getAmc(), e.getAmfiCode(),
				e.getAmount(), e.getIsin(), e.getIsinDescription(), e.getLockInDays(), e.getLockInFlag(), e.getMode(),
				e.getNarration(), e.getNav(), e.getNavDate(), e.getRegistrar(), e.getSchemeCode(), e.getSchemePlan(),
				e.getTransactionDate(), e.getTxnId(), e.getType(), e.getUcc(), e.getUnits());
	}
	...
	private ClientConsentMappingDTO toClientConsentDTO(ClientConsentMappingEntity e) {
		return new ClientConsentMappingDTO(e.getId(), e.getClientCode(), e.getPan(), e.getDob(), e.getEmail(),
