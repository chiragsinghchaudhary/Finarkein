...
public boolean checkValidConsent(GetRequest getRequest) {
    ...
    ClientConsentMappingEntity entity = clientConsentRepository.getlatestClientConsentObject(
            pan, Constants.CONSENT, Constants.SUCCESS, Constants.ACTIVE);

    long duration = System.currentTimeMillis() - start;
    logger.info("checkValidConsent completed -> pan={} | duration={} ms", pan, duration);

    if (entity != null) {
        logger.debug("Consent record found -> state={} | consentStatus={}", entity.getState(), entity.getConsentStatus());
...
public GetResultResponse getDBRecords(GetRequest getRequest) {
    ...
    // Combined debug log
    logger.debug("Fetched DB records -> DepositHolder={} | DepositSummary={} | DepositTransaction={} | EquityHolder={} | EquitySummary={} | EquityTransaction={} | MFHolder={} | MFSummary={} | MFTransaction={}",
        depositHolders.size(), depositSummaries.size(), depositTransactions.size(),
        equityHolders.size(), equitySummaries.size(), equityTransactions.size(),
        mfHolders.size(), mfSummaries.size(), mfTransactions.size());
...
public ConsentNewRunResponse createNewRun(String workspace, String flowId,
    ...
    logger.info("API call completed: createNewConsentRun -> pan={} | duration={} ms",
            pan, System.currentTimeMillis() - apiStart);
    ...
    logger.info("DB save completed -> pan={} | requestId={} | duration={} ms",
            savedEntity.getPan(), savedEntity.getRequestId(), System.currentTimeMillis() - dbStart);

    // --- Completion ---
    logger.info("createNewRun completed -> pan={} | totalDuration={} ms",
            pan, System.currentTimeMillis() - overallStart);
...
public RecurringNewRunResponse createNewRunFetch(String workspace, String flowId, GetRequest getRequest) {
    ...
    logger.info("DB fetch completed -> pan={} | duration={} ms",
            pan, System.currentTimeMillis() - dbFetchStart);
    ...
    logger.info("API call completed: createNewRecurringRun -> pan={} | duration={} ms",
            pan, System.currentTimeMillis() - apiStart);
    ...
    logger.info("DB save completed -> pan={} | requestId={} | duration={} ms",
            entity.getPan(), entity.getRequestId(), System.currentTimeMillis() - dbSaveStart);

    // --- Completion ---
    logger.info("createNewRunFetch completed -> pan={} | totalDuration={} ms",
            pan, System.currentTimeMillis() - overallStart);
...
public GetStatusResponse getStatus(String workspace, String flowId, String requestId) {
    ...
    logger.info("API call completed: getStatus -> requestId={} | duration={} ms",
            requestId, System.currentTimeMillis() - apiStart);
    ...
    clientConsentRepository.updateStatus(
            statusResponse.getState().getState(),
            statusResponse.getState().getConsentStatus(),
            statusResponse.getState().getDataFetchStatus(),
            requestId);

    logger.info("DB updateStatus completed -> requestId={} | duration={} ms",
            requestId, System.currentTimeMillis() - dbStart);

    // --- Completion ---
    logger.info("getStatus completed -> requestId={} | totalDuration={} ms",
            requestId, System.currentTimeMillis() - overallStart);
...
public GetResultResponse getResult(String workspace, String flowId, String requestId) {
    ...
    logger.info("API call completed: getResult -> requestId={} | duration={} ms",
            requestId, System.currentTimeMillis() - apiStart);
    ...
    logger.info("DB fetch completed -> requestId={} | duration={} ms",
            requestId, System.currentTimeMillis() - dbFetchStart);
    ...
    if (Constants.SUCCESS.equals(resultResponse.getState().getDataFetchStatus())) {
        ...
        logger.info("DB replace operation completed -> pan={} | duration={} ms",
                pan, System.currentTimeMillis() - dbSaveStart);

    } else {
        logger.warn("Data fetch status is NOT SUCCESS -> requestId={} | status={}",
                requestId, resultResponse.getState().getDataFetchStatus());
    }

    logger.info("getResult completed -> requestId={} | totalDuration={} ms",
            requestId, System.currentTimeMillis() - overallStart);
...
private ClientConsentMappingDTO mergeConsentRequestAndResponse(ConsentNewRunRequest request,
    ...
    if (request == null || request.getUser() == null || response == null) {
        logger.error("mergeConsentRequestAndResponse: Null parameter(s) detected. Request={}, Response={}",
                request, response);
...
private ClientConsentMappingDTO mergeRecurringRequestAndResponse(
        ClientConsentMappingEntity clientConsentMappingEntity,
        RecurringNewRunRequest request,
...
private ConsentNewRunResponse mapConsentEntityToResponse(ClientConsentMappingEntity entity) {
    ...
    logger.debug(
            "Mapping ClientConsentMappingEntity to ConsentNewRunResponse | requestId={} | consentHandle={}",
            entity.getRequestId(),
...
private List<DepositTransaction> mapDepositTransactionEntities(List<DepositTransactionEntity> entities) {
    ...
    logger.debug("Mapping {} DepositTransactionEntity records to DepositTransaction response objects", entities.size());
    ...
    logger.debug("Mapped {} DepositTransactionEntity records to DepositTransaction response objects", responseList.size());
    return responseList;
}
...
private List<EquityTransaction> mapEquityTransactionEntities(List<EquityTransactionEntity> entities) {
    ...
    logger.debug("Mapping {} EquityTransactionEntity records to EquityTransaction response objects", entities.size());
